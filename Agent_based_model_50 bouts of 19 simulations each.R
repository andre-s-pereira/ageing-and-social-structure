...
nbouts <- 50                                              # number of bouts 
nsims <- 19                                               # number of simulations 
...
noldmin <- 2                                              # minimum number of old individuals
noldmax <- 10                                             # maximum number of old individuals
outputdata <- data.frame(nbout=rep(1:nbouts,each=nsims),  # storage for output
                         nsim=rep(1:nsims,nbouts),
...
for(curbout in 1:nbouts){ # for each bout (curbout = current bout)
  
  for(cursim in 1:nsims){ # for each simulation (cursim = current simulation)
    
    print(cursim+(curbout*19-19)) # print the current simulation number to the terminal, so that we can follow the progress of the run
    ...
    outputdata[cursim+(curbout*19-19), 'n.old.individuals'] <- nold             # store it in the output
    outputdata[cursim+(curbout*19-19), 'prop.old.individuals'] <- nold/n        # store the proportion of old individuals in the output
    outputdata[cursim+(curbout*19-19), 'perc.old.individuals'] <- (nold/n)*100  # store the percentage of old individuals in the output
    ...
    # global network metrics
     outputdata[cursim+(curbout*19-19), 'mean.degree'] <- mean(curdegrees)
     outputdata[cursim+(curbout*19-19), 'transitivity'] <- transitivity(inet, type = "global") 
     outputdata[cursim+(curbout*19-19), 'diameter'] <- diameter(inet, unconnected = T) 


  } ## end of for loop
} ## end of bout
save("outputdata", file="outputdata_50 bouts of 19 networks.Rdata")
